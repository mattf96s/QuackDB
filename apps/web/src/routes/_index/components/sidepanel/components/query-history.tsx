...
export default function QueryHistory() {
	...
	return (
		<div className="flex flex-col pt-2">
			...
			{runs.length === 0 && (
				<div className={cn("py-2 pl-6 text-gray-400 text-sm")}>No runs yet</div>
			)}
			...
			>
				{runs.map((run) => {
					...
					return <RunHoverCard key={key} {...run} />;
				})}
			</motion.div>
		</div>
	);
		...
		>
			<div className="min-w-0 flex-auto">
				<div className="flex items-center gap-x-3">
					...
					<h2 className="min-w-0 font-semibold text-sm leading-6 dark:text-white">
						<span className="truncate">{error ? "Error" : "Success"}</span>
						<span className="px-1 text-gray-400">/</span>
						<span className="whitespace-nowrap">{duration}s</span>
						<span className="absolute inset-0" />
					</h2>
					<Badge variant="outline" color={cacheHit ? "green" : "blue"}>
						{cacheHit ? "CACHE" : "LIVE"}
					</Badge>
				</div>
				{error && (
					<div className="mt-3 flex items-center gap-x-2.5 text-gray-400 text-xs leading-5">
						<span className="line-clamp-3 truncate text-wrap break-words text-left font-mono text-gray-700 text-xs dark:text-gray-100">
							{error}
						</span>
					</div>
				)}
				<div className="mt-3 flex items-center gap-x-2.5 text-gray-400 text-xs leading-5">
					...
					{isCopied && (
						<div className="absolute inset-y-1 right-1">
							<CopyCheck className="bg-transparent text-green-700" size={18} />
						</div>
					)}
				</div>
			</div>
