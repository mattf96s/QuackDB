...
export default function DataSources() {
	...
	return (
		<div className="flex w-full flex-col pt-2">
			...
			<div
				className={cn(
					"flex w-full flex-col space-y-1 py-1 pr-8 pl-4",
					isCollapsed && "hidden",
				)}
			>
				{sources.map((source) => (
					<DatesetItem key={source.path} {...source} />
				))}
			</div>
		</div>
	);
		...
		<>
			<ContextMenu key={path}>
				<ContextMenuTrigger className="w-full data-[state=open]:bg-gray-100 data-[state=open]:dark:bg-gray-900">
					...
					>
						<div className="relative inline-flex w-full items-center gap-1">
							...
							{isCopied && (
								<span className="absolute inset-y-0 top-0.5 right-0">
									<CopyCheck
										size={16}
				...
				<ContextMenuContent className="w-64">
					<ContextMenuItem inset onSelect={onCopy}>
						Insert Table SQL
					</ContextMenuItem>
					...
					<ContextMenuItem inset onSelect={onDownloadHandler}>
						Download
					</ContextMenuItem>
					<ContextMenuSeparator />
					<ContextMenuItem inset onSelect={() => setShowDelete(true)}>
						Delete
					</ContextMenuItem>
				</ContextMenuContent>
			</ContextMenu>
...
function SourcesToolbar() {
	...
	return (
		<>
			<DropdownMenu>
				...
				>
					...
					<DropdownMenuGroup>
						...
						>
							Paste
							<span className="ml-2 rounded-full border border-gray-500 bg-transparent px-2.5 py-0.5 font-semibold text-gray-500 text-xs">
								soon
							</span>
						</DropdownMenuItem>
					</DropdownMenuGroup>
			...
			{/* content dialog */}
			<AddDataModal isOpen={showAddDataModal !== "OFF"} onOpenChange={onClose}>
